...
import { X, ZoomIn, ChevronLeft, ChevronRight, Images, ImageOff, ArrowRight } from 'lucide-react'
...
const SKELETON_HEIGHTS = ['h-52', 'h-64', 'h-48', 'h-72', 'h-56', 'h-60']
    ...
    >
      <div className="relative flex items-center justify-center">
        ...
        <div className="w-16 h-16 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center relative z-10">
          <ImageOff className="w-7 h-7 text-primary" />
        </div>
      </div>
      <div className="space-y-1.5">
        ...
        <p className="text-muted-foreground text-sm max-w-xs leading-relaxed">
          {isFiltered
            ? `Nothing in the ${category} category — try a different filter.`
            : 'Beautiful moments are being captured — check back soon.'}
        </p>
      </div>
    </motion.div>
  )
...
export default function Gallery() {
  ...
  const totalCount = activeFilter === 'All'
    ? photos.length
    : photos.filter(p => p.category === activeFilter).length
  ...
  return (
    <section id="gallery" className="relative w-full py-20 md:py-28 bg-background text-foreground overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-12">
          ...
          <div className="flex items-center space-x-2 bg-foreground/5 border border-foreground/10 rounded-full px-4 py-2 self-start md:self-auto">
            ...
            <span className="text-xs text-muted-foreground font-medium">
              {loading ? '—' : `${filtered.length}${totalCount > PREVIEW_LIMIT ? `/${totalCount}` : ''} photo${filtered.length !== 1 ? 's' : ''}`}
            </span>
          </div>
        </div>
        ...
        {!loading && totalCount > PREVIEW_LIMIT && (
          ...
          >
            <Link
              to="/gallery"
              className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full border border-primary/30 bg-primary/8 text-primary hover:bg-primary hover:text-white transition-all duration-300 text-sm font-semibold"
            >
              +{totalCount - PREVIEW_LIMIT} more photos
